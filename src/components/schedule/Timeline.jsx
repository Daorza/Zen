...
function formatDate(date) {
  ...
  if (selected === normalize(today)) return "Hari Ini";

  if (selected === normalize(tomorrow)) return "Besok";

  if (selected === normalize(yesterday)) return "Kemarin";
      ...
      {loading && (
        <div className="flex flex-col gap-4">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="grid grid-cols-[70px_20px_1fr] gap-4 animate-pulse"
            >
